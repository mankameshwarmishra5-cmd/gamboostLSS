...
% \VignetteDepends{gamboostLSS, R2BayesX, BayesX, survival}
...
if (all(is.na(pd))){
    stop(sQuote("gamboostLSS"), "not properly installed.")
} else {
    if (compareVersion(pd$Version, "1.2-0") < 0){
        stop(sQuote("gamboostLSS"), " (1.2-0 or newer) needs to be installed!")
    }
}

## for the exact reproduction of the plots R2BayesX >= 0.3.2 is needed
if (!require("R2BayesX"))
  stop("cannot attach package ", sQuote("R2BayesX"))

if (!require("BayesX"))
  stop("cannot attach package ", sQuote("BayesX"))
...
\Abstract{ \emph{This vignette is a slightly modified version of
  ...
  available from CRAN (\url{https://CRAN.R-project.org/package=gamboostLSS}).}
...
(\url{https://cran.r-project.org/package=gamboostLSS}). Current development
versions are hosted on GitHub (\url{https://github.com/boost-R/gamboostLSS}). As
...
downloaded after registration from \url{https://www.measuredhs.com}. For
...
available at \url{https://www.gamlss.com} and the documentation of the
